...
class UpiBottomSheet : BottomSheetDialogFragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mUri = Uri.parse(requireArguments().getString("uri"))
        mUpiApps = requireArguments().getStringArrayList("ARG_UPI_APPS_LIST")!!
    }
    ...
    private fun getAllUpiApps() {
        ...
        val appList = requireContext().packageManager.queryIntentActivities(paymentIntent, 0)


        if(mUpiApps.isNotEmpty()){
            val finalList: MutableList<ResolveInfo> = mutableListOf()
            for(i in appList){
                if(mUpiApps.contains(i.loadLabel(requireContext().packageManager).toString().toLowerCase()))
                    finalList.add(i)
            }
